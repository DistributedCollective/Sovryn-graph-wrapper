...
async function getBlockNumberFromDate(timestamp: number): Promise<number> {
  ...
  return data.transactions[0].blockNumber;
}
...
const parseData = (
  ...
  return sorted.map((item) => {
    ...
    if (
      currentData.connectorTokens[0].token.symbol === currentData.token1.symbol
    ) {
      ...
      currentData.connectorTokens[0].token.symbol === currentData.token0.symbol
    ) {
      currentBaseVolume = parseFloat(
        currentData.connectorTokens[1].totalVolume
      );
