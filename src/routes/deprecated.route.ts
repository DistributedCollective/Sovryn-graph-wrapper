...
router.get(
  ...
  asyncMiddleware(async (req: Request, res: Response, next: NextFunction) => {
    try {
      req.log.info(req, 'get deprecated endpoint')
      const errors = validationResult(req)
      if (!errors.isEmpty()) {
        req.log.debug(errors.array(), 'handling message errors')
        throw new InputValidateError(errors.array())
      ...
      const response = {
        error: true,
        message:
          'This API endpoint has now been deprecated. If you need help, please reach out to the Sovryn team via our discord: https://discord.com/channels/729675474665603133/',
        statusCode: 410
      }
      res.status(410).json(response)
    } catch (error) {
